...
| `inspect_element` | `selector: string`, `properties?: string[]` | Full element details — tag, attributes, bounding rect + inViewport, curated computed styles, overflow info (clipped/overflowing content), effective visibility (`visible` + reason, accounting for ancestors), occlusion (`hittable` + `occludedBy`), accessibility (`a11y`: role, name, states), live form state; `properties` reads extra CSS props by name |
