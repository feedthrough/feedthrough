...
| `inspect_element(selector, properties?)` | Tag, attributes, full bounding rect + inViewport, curated computed styles, overflow info (clipped/overflowing content), effective visibility (`visible` + reason, accounting for ancestors), live form state; `properties` reads extra CSS props by name |
